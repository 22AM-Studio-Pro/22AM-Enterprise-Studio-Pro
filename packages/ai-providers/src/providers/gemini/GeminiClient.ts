...
export class GeminiClient {
  ...
  constructor(config: GeminiConfig, httpClient?: HttpClient, retry?: RetryPolicy) {
    this.base = config.apiBaseUrl ?? 'https://api.gemini.example/v1'
  ...
  async post(path: string, apiKey: string, body: any) {
    return this.retry.run(async () => {
      ...
      if (res.status >= 400) {
        const err: any = new Error(json?.error?.message || `Gemini error ${res.status}`)
